...
window.renderExpenses = () => { 
    ...
    [...filtered].reverse().forEach((exp) => { 
        const li = document.createElement('li'); li.className = 'expense-item ex-' + exp.account; 
...
window.renderBalances = () => { 
    const balFacebank = document.getElementById('balFacebank'); if(!balFacebank) return;
    balFacebank.innerText = '$' + parseFloat(state.balances.facebank).toFixed(2); 
    document.getElementById('balBinance').innerText = '₮' + parseFloat(state.balances.binance).toFixed(2); 
    document.getElementById('balBs').innerText = 'Bs. ' + parseFloat(state.balances.bs).toFixed(2); 
...
window.fetchExchangeRates = async () => { 
    const btn = document.getElementById('btnSyncRates'); if(btn) btn.innerText = '📡 Sync...'; 
    try { 
        ...
        const dataBcv = await fetchWithFallback('https://ve.dolarapi.com/v1/dolares/oficial'); if (dataBcv) bcvVal = dataBcv.promedio || 0; 
        const dataEur = await fetchWithFallback('https://ve.dolarapi.com/v1/euros/oficial'); if (dataEur) euroVal = dataEur.promedio || 0; 
        ...
        const rBcv = document.getElementById('rateBcv');
        if(rBcv) {
            rBcv.innerText = `Bs. ${parseFloat(bcvVal).toFixed(2)}`; 
            ...
            window.renderBalances(); window.renderExpenses(); btn.innerText = '📡 Sincronizar'; 
        }
